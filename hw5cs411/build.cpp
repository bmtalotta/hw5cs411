...
using std::vector;
using std::max;
//basic idea for code based on RobotCoinCollection from textbook pg.289 Introduction to the Designand Analysis of Algorithms
	...
	for (int col = 0; col < w; ++col) {
		for (int row = 0; row < e; ++row) {
			if (col == 0 && row == 0) {
				...
				if (col != 0 && row != 0) {
					recVals[col][row] = max(max(recVals[col - 1][row], recVals[col-1][row-1] + organizeBridges[col][row]), max(recVals[col][row - 1], recVals[col - 1][row - 1] + organizeBridges[col][row]));
				}
				else if (col != 0) {
